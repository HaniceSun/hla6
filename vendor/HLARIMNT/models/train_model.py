...
class ModelTrainer:
    ...
    def run(self) -> None:
        ...
        save_dir = f'{self.data_dir}/{self.params["exp_name"]}'
        model_save_loc = 'model'

        self.hla_info = path_to_dict(hla_info_loc)
        if dataset in ['Mixed','Equal','Ind_Pan-Asian','Ind_T1DGC','T1DGC_530','T1DGC_1300','T1DGC_2600']:
            self.freq_info = pd.read_csv(freq_info_loc ,index_col=1)
        else:
            self.freq_info = pd.read_table(freq_info_loc, sep='\t|\s+',engine='python', index_col=1)
        ...
        for idx in model_cfg:
            ...
            for digit in digit_list:
                ...
                if digit != '2-digit':
                    self._load_model(idx, digit, save_dir + '/' + model_save_loc)
                ...
                if self.params['fold_num'] != -1:
                    ...
                    for key in self.models.keys():
                        self.models[key].load_state_dict(torch.load(save_dir + '/' + model_save_loc + f'/{idx}-{digit}' + f'/{key}.pth'))
